...
export class MeetingsService {
  ...
  addAttendee(meetingId: number, attendeeEmail: string, token: string): Observable<Meeting> {
    const url = `${this.apiUrl}/${meetingId}?action=add_attendee&email=${encodeURIComponent(attendeeEmail)}`;
    return this.http.patch<Meeting>(url, {}, {
      headers: new HttpHeaders().set('Authorization', `Bearer ${token}`)
    });
  }

  leaveMeeting(meetingId: number, token: string): Observable<Meeting> {
  ...
  addMeeting(payload: any): Observable<Meeting> {
    return this.http.post<Meeting>(`${this.newApiUrl}`, payload);
  }
